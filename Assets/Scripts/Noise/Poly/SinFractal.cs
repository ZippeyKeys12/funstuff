namespace ZNoise
{
    class SinFractal : Generator
    {
        ...
        public override Sample1D Get(float x, float frequency)
        {
            ...
            {
                noiseHeight += 2 * Maths.Abs(operands[i].Get(x, freq) * spectralWeights[i]) - 1;
        ...
        public override Sample2D Get(float x, float y, float frequency)
        {
            ...
            for (var i = 0; i < operands.Length; i++)
            {
                noiseHeight += 2 * Maths.Abs(operands[i].Get(x, y, freq) * spectralWeights[i]) - 1;
                maxNoiseHeight += spectralWeights[i];
        ...
        public override Sample3D Get(float x, float y, float z, float frequency)
        {
            ...
            for (var i = 0; i < operands.Length; i++)
            {
                noiseHeight += 2 * Maths.Abs(operands[i].Get(x, y, z, freq) * spectralWeights[i]) - 1;
                maxNoiseHeight += spectralWeights[i];
